...
```{r}
if (!file.exists("rdata/acm_with_act.RData")) {
  ...
  save(acm_with_act, file = "rdata/acm_with_act.RData")
}
```
...
```{r}
if (!file.exists("rdata/acm_with_out_act.RData")) {
  variables_qualitatives <- subset(g_data, select = -c(département, commune, canton, x, y))
  acm_with_out_act <- MCA(variables_qualitatives, graph = FALSE)
  save(acm_with_out_act, file = "rdata/acm_with_out_act.RData")
}
```
...
La fonction ``catdes`` en R permet de décrire les clusters obtenus par une méthode de clustering, en analysant la répartition des variables catégorielles dans ces clusters. Les résultats de cette fonction permettent de comprendre à partir de quelles caractéristiques linguistiques ces clusters sont formés :

1. **Cla/Mod** (Classe/Modalité) : 
   - Cette colonne représente le pourcentage de la modalité de la variable qui se trouve dans le cluster donné. Autrement dit, c'est la proportion de cette modalité dans le cluster par rapport à l'ensemble de cette modalité dans la population totale.

2. **Mod/Cla** (Modalité/Classe) :
   - Cette colonne représente le pourcentage de la modalité parmi les individus du cluster. C'est la proportion d'individus ayant cette modalité dans le cluster par rapport au total des individus dans le cluster.

3. **Global p.value** :
   - La valeur p globale (p-value) évalue la signification statistique de la répartition de la modalité de la variable dans le cluster. Une petite valeur p (typiquement inférieure à 0,05) indique que la modalité de la variable est significativement plus (ou moins) fréquente dans ce cluster que ce à quoi on pourrait s'attendre par hasard.

4. **v.test** :
   - Le test v (v-test) est une statistique qui compare la proportion observée de la modalité dans le cluster à la proportion attendue (en supposant une répartition uniforme). Une valeur de test v élevée (en valeur absolue) indique que la modalité est particulièrement sur-représentée (ou sous-représentée) dans ce cluster. Une valeur positive indique une sur-représentation et une valeur négative indique une sous-représentation. La statistique v-test suit approximativement une distribution normale, donc des valeurs en dehors de [-1.96, 1.96] correspondent à des p-values inférieures à 0.05, indiquant une significativité statistique.

Nous allons analyser le cluster numéro 1 avec k = 4 pour le KNN avec les données qui prennent en compte les accents pour une meilleure compréhension. On a les résultats suivants :

**Cluster 1**

Cla/Mod (Classe/Modalité) :

- Finit_par_éei=e : 89.71% des mots qui se terminent par "e" appartiennent à ce cluster

- Finit_par_éei=autres : 10.20% des mots qui se terminent par "autres" appartiennent à ce cluster

- Finit_par_éei=i : 1.57% des mots qui se terminent par "i" appartiennent à ce cluster

Mod/Cla (Modalité/Classe) :

- Finit_par_éei=e : 48.30% des mots dans ce cluster se terminent par "e"

- Finit_par_éei=autres : 1.46% des mots dans ce cluster se terminent par "autres"

- Finit_par_éei=i : 1.26% des mots dans ce cluster se terminent par "i"

Global p.value :

- Finit_par_éei=e : 4.05e-208, indiquant une très forte signification statistique

- Finit_par_éei=autres : 1.90e-11, indiquant une forte signification statistique

- Finit_par_éei=i : 2.62e-153, indiquant une très forte signification statistique

v.test :

- Finit_par_éei=e : 30.79, indiquant une sur-représentation significative de cette modalité

- Finit_par_éei=autres : -6.71, indiquant une sous-représentation significative de cette modalité

- Finit_par_éei=i : -26.38, indiquant une sous-représentation significative de cette modalité

Ces mesures aide à comprendre quelles caractéristiques définissent chaque cluster, en identifiant quelles modalités de variables catégorielles sont sur-représentées ou sous-représentées dans chaque cluster.

Ainsi dans cette section, nous avons exploré la relation entre les clusters formés à partir de données textuelles à l'aide de la méthode KNN et les variables catégorielles en utilisant des matrices de similarité. Ces matrices ont été créées en calculant les similarités entre les données textuelles et les variables catégorielles, ce qui nous a permis de quantifier les relations entre ces différentes entités.
...
Measuring the diffusion of linguistic change, John Nerbonne, https://royalsocietypublishing.org/doi/abs/10.1098/rstb.2010.0048

Classification automatique - clustering, cours de Mme Marie Chavent

https://irem.univ-lille.fr/~site/IMG/pdf/142_distance_entre_les_mots.pdf

https://lrouviere.github.io/TUTO_VISU/shiny.html 

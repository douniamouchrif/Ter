...
ui <- fluidPage(
  ...
  tabsetPanel(
    ...
    tabPanel("CAH",
             ...
             p("La classification ascendante hiérarchique (CAH) est une méthode de clustering qui construit une hiérarchie en regrouppant progressivement les points de données les plus similaires, en commençant par chaque point comme un cluster individuel jusqu'à former un seul cluster qui englobe tous les points."),
             ...
             p("Cette technique se base sur la similarité ou la distance entre les données pour fusionner les clusters les plus proches. C'est pourquoi, avant d'appliquer les différentes méthode de CAH, nous avons construit des matrices de dissimilarité. Comme nous devons comparer des mots, nous avons choisi 2 distances pour mesurer la distance linguistique entre les communes."),
             ...
             p("La distance de Levenshtein quant à elle, mesure le nombre minimum d'opérations nécessaires (insertions, suppressions et remplacements) pour passer d'une chaîne de caractère à l'autre."),
             p("Une fois les ditances choisies, nous avons donc constrit une matrice de distance pour chaque mot, puis nous avons fait la somme de toutes ces matrices. Nous obtenions ainsi la matrice finale (de taille 3061x3061), contenant la distance linguistqiue entre chaque commune par rapport à l'ensemble du texte. Nous avons utilisé la fonction stringdistmatrix du package stringdist en spécifiant en paramètre la distance (jaccard ou levenshtein)."),

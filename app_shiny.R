...
ui <- fluidPage(
  ...
  tabsetPanel(
    ...
    tabPanel("Analyse des clusters",
             ...
             h3("Conclusion"),
             p("La fonction catdes en R permet de décrire les clusters obtenus par une méthode de clustering, en analysant la répartition des variables catégorielles dans ces clusters. Les résultats de cette fonction permettent de comprendre à partir de quelles caractéristiques linguistiques ces clusters sont formés :"),
             h4("Cla/Mod (Classe/Modalité) :"),
             p(" - Cette colonne représente le pourcentage de la modalité de la variable qui se trouve dans le cluster donné. Autrement dit, c’est la proportion de cette modalité dans le cluster par rapport à l’ensemble de cette modalité dans la population totale."),
             h4("Mod/Cla (Modalité/Classe) :"),
             p(" - Cette colonne représente le pourcentage de la modalité parmi les individus du cluster. C’est la proportion d’individus ayant cette modalité dans le cluster par rapport au total des individus dans le cluster."),
             h4("Global p.value :"),
             p(" - La valeur p globale (p-value) évalue la signification statistique de la répartition de la modalité de la variable dans le cluster. Une petite valeur p (typiquement inférieure à 0,05) indique que la modalité de la variable est significativement plus (ou moins) fréquente dans ce cluster que ce à quoi on pourrait s’attendre par hasard."),
             h4("v.test :"),
             p(" - Le test v (v-test) est une statistique qui compare la proportion observée de la modalité dans le cluster à la proportion attendue (en supposant une répartition uniforme). Une valeur de test v élevée (en valeur absolue) indique que la modalité est particulièrement sur-représentée (ou sous-représentée) dans ce cluster. Une valeur positive indique une sur-représentation et une valeur négative indique une sous-représentation. La statistique v-test suit approximativement une distribution normale, donc des valeurs en dehors de [-1.96, 1.96] correspondent à des p-values inférieures à 0.05, indiquant une significativité statistique."),
             p("Nous allons analyser le cluster numéro 1 avec k = 4 pour le KNN avec les données qui prennent en compte les accents pour une meilleure compréhension. On a les résultats suivants :"),
             h4("Cluster 1"),
             p("Cla/Mod (Classe/Modalité) :"),
             p(" - Finit_par_éei=e : 89.71% des mots qui se terminent par “e” appartiennent à ce cluster"),
             p(" - Finit_par_éei=autres : 10.20% des mots qui se terminent par “autres” appartiennent à ce cluster"),
             p(" - Finit_par_éei=i : 1.57% des mots qui se terminent par “i” appartiennent à ce cluster"),
             h4("Mod/Cla (Modalité/Classe) :"),
             p("Finit_par_éei=e : 48.30% des mots dans ce cluster se terminent par “e”"),
             p("Finit_par_éei=autres : 1.46% des mots dans ce cluster se terminent par “autres”"),
             p("Finit_par_éei=i : 1.26% des mots dans ce cluster se terminent par “i”"),
             h4("Global p.value :"),
             p(" - Finit_par_éei=e : 4.05e-208, indiquant une très forte signification statistique"),
             p(" - Finit_par_éei=autres : 1.90e-11, indiquant une forte signification statistique"),
             p(" - Finit_par_éei=i : 2.62e-153, indiquant une très forte signification statistique"),
             h4("v.test :"),
             p(" - Finit_par_éei=e : 30.79, indiquant une sur-représentation significative de cette modalité"),
             p(" - Finit_par_éei=autres : -6.71, indiquant une sous-représentation significative de cette modalité"),
             p(" - Finit_par_éei=i : -26.38, indiquant une sous-représentation significative de cette modalité"),
             p("Ces mesures aide à comprendre quelles caractéristiques définissent chaque cluster, en identifiant quelles modalités de variables catégorielles sont sur-représentées ou sous-représentées dans chaque cluster."),
             p("Ainsi dans cette section, nous avons exploré la relation entre les clusters formés à partir de données textuelles à l’aide de la méthode KNN et les variables catégorielles en utilisant des matrices de similarité. Ces matrices ont été créées en calculant les similarités entre les données textuelles et les variables catégorielles, ce qui nous a permis de quantifier les relations entre ces différentes entités."),
             p("Nous avons constaté que la fonction catdes est un outil précieux pour mettre en lumière les associations entre les clusters et les variables catégorielles. Cela a grandement enrichi notre compréhension de la répartition des dialectes dans la région étudiée, en nous permettant de mieux comprendre les tendances et les structures sous-jacentes des données linguistiques."),
    ),
